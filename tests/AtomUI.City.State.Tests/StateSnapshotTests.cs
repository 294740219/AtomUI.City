...
public sealed class StateSnapshotTests
{
    ...
    [Fact]
    public void SnapshotRestoreAppliesNullReferenceValues()
    {
        var key = new StateKey<string?>("AtomUI.City.Tests.OptionalTheme");
        var registry = new ApplicationStateRegistry();
        registry.Add(StateDefinition.Create(key, "light", snapshotPolicy: StateSnapshotPolicy.Persisted));
        var snapshot = new StateSnapshot(
            [
        ...
        Assert.Null(registry.Get(key).Value);
        Assert.Equal(3, registry.Get(key).Version);
    }
    ...
    [Fact]
    public void SnapshotRestoreRejectsTransientStatePolicy()
    {
        var diagnostics = new InMemoryHostDiagnostics();
        var key = new StateKey<string>("AtomUI.City.Tests.TransientTheme");
        var registry = new ApplicationStateRegistry(diagnostics);
        registry.Add(StateDefinition.Create(key, "light", snapshotPolicy: StateSnapshotPolicy.Transient));
        var snapshot = new StateSnapshot(
            [
                new StateSnapshotEntry(
                    key.Name,
        ...
        Assert.Equal(HostDiagnosticSeverity.Warning, record.Severity);
        Assert.Contains(key.Name, record.Message, StringComparison.Ordinal);
        Assert.Contains("snapshot policy", record.Message, StringComparison.OrdinalIgnoreCase);
    }

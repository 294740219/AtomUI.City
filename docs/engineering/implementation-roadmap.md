# AtomUI.City 实现顺序治理规范

版本：v0.1
状态：正式初版
适用范围：从设计文档进入代码实现的阶段顺序、验收标准、测试门禁和暂停条件。

本文件不是进度跟踪文档，不记录当前完成状态，也不参与 1.0 发布完成度统计。AtomUI.City 1.0 是否可以发布，只以 [全局 1.0 进度](../superpowers/plans/2026-06-11-development-tracking-plan.md) 为准。

## 1. 目标

本规范用于约束 AtomUI.City 的实现顺序。
...
任一功能点在 [全局 1.0 进度](../superpowers/plans/2026-06-11-development-tracking-plan.md) 勾选前必须满足：

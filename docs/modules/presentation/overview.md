...
连接 Routing、MVVM、State、Localization、Security 与 Avalonia/AtomUI 的 UI 运行时桥接层。
...
`tests/AtomUI.City.Presentation.Tests` 必须覆盖 [features.md](features.md) 中每个 Feature ID。产品级完成不能只看现有测试文件存在，必须补齐 [testing.md](testing.md) 中列出的必断言行为。

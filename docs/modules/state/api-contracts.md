...
| StateCollectionSnapshotEntry<TKey,TItem> constructor / Key init | 创建集合快照条目并保护 key init 边界。 | key 不得为 null；itemVersion 必须大于等于 0。 | StateCollectionSnapshotEntry<TKey,TItem>。 | key 为 null 抛 `ArgumentNullException`；itemVersion 小于 0 抛 `ArgumentOutOfRangeException`。 | 无。 | record init 属性不得绕过已实现的集合快照条目边界。 |

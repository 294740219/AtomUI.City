...
| StateSnapshotEntry constructor / StateName init | 创建快照条目并保护状态名 init 边界。 | stateName/valueType/version/schemaVersion 必须有效。 | StateSnapshotEntry。 | stateName 为 null 或空白抛 `ArgumentException`；version 小于 0 或 schemaVersion 小于 1 抛 `ArgumentOutOfRangeException`。 | 无。 | record init 属性不得绕过已实现的快照条目边界。 |

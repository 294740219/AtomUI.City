...
| StateCollectionChange<TKey,TItem> constructor / Kind init / Key init | 创建集合变更记录并保护 identity init 边界。 | kind 必须为已定义枚举；key 不得为 null；collectionVersion/itemVersion 必须大于等于 0。 | StateCollectionChange<TKey,TItem>。 | 未知 kind、负 collectionVersion 或负 itemVersion 抛 `ArgumentOutOfRangeException`；key 为 null 抛 `ArgumentNullException`。 | 无。 | record init 属性不得绕过已实现的集合变更边界。 |

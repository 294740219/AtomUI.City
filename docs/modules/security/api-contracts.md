...
| ICommandAuthorizationSource.GetStateAsync / CheckExecutionAsync | 将 command descriptor 转成授权状态或执行前授权结果。 | CommandAuthorizationContext；descriptor provider 可按 command id 返回 policy、未授权行为和 denied message key。 | CommandAuthorizationState 或 AuthorizationResult。 | 无 descriptor 返回 Allowed；Challenge/Forbidden/Failed 让 CanExecute=false；Hide 策略让 IsVisible=false；descriptor provider 或 evaluator 异常返回 Failed/EvaluatorFailed。 | 必须观察 token；取消返回 Cancelled state/result。 | 订阅认证状态、descriptor 和 permission registry 变化；Dispose 后释放订阅，不再发布 AuthorizationChanged。 |
| IAccessTokenProvider.GetTokenAsync | 获取 access token。 | AccessTokenRequest 包含 resource、scheme 和 operation。 | AccessTokenResult，状态包含 None、Success、Required、Expired、Failed、Unavailable、Cancelled。 | token 缺失、不需要 token、刷新失败、provider 不可用和取消都有独立 status；DelegateAccessTokenProvider 异常返回 Failed 并保存 Exception。 | 必须观察 token；预取消不调用 delegate。 | 并发 refresh 合并或拒绝由具体 provider 明确；Data bridge 在 transport 前调用并把 Failed/Unavailable 映射为 credential unavailable。 |

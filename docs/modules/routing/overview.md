...
源码：`src/AtomUI.City.Routing`
测试：`tests/AtomUI.City.Routing.Tests`

## 模块定位

业务无关的桌面导航系统，负责 Route -> ViewModel Target。
...
`tests/AtomUI.City.Routing.Tests` 必须覆盖 [features.md](features.md) 中每个 Feature ID。产品级完成不能只看现有测试文件存在，必须补齐 [testing.md](testing.md) 中列出的必断言行为。

...
- 输出不符合 contract：测试失败。
- 相同规范化目标目录的进程内 render 使用 keyed gate 串行化；gate 引用归零后移除，不能按历史路径无限增长。
- 不同目标目录可以并行；跨进程不共享 gate，必须以 `FileMode.CreateNew` 保证文件不被覆盖。
- 写入前完成全量目标冲突预检；出现 IO 失败时只回滚本次调用创建的文件，并仅删除已空的本次目录。

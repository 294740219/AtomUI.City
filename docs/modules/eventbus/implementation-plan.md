...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AUC-EVENTBUS-001 | 是 | 是 | EventPublicationTests | 已有基线 | 部分通过 | 断言 delivery/post result 边界、null event、预取消 token、publish options 边界、result immutable/null delivery、error policy、diagnostics。 | 仍需完整产品合同测试 | 产品化进行中 |
| AUC-EVENTBUS-002 | 是 | 是 | EventSubscriptionTests | 已有基线 | 部分通过 | 断言 dispose 后不再收到事件、StopAsync 移除新发布快照、等待 in-flight handler、owner stop/cancellation 释放、已 Disposed 后 StopAsync 幂等。 | 仍需完整产品合同测试 | 产品化进行中 |

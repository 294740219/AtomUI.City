...
| EventContext<TEvent> constructor | 创建 handler 执行上下文。 | eventData 非 null，contractId 必须已创建，eventId 必须非空，correlationId 必须为稳定 id，不得为空白、包含首尾空白或控制字符。 | EventContext<TEvent>。 | eventData 为 null 抛 `ArgumentNullException`；contractId 为 default、eventId 为空或 correlationId 无效抛 `ArgumentException`。 | 仅保存 token，不主动取消。 | 创建后不可变。 |

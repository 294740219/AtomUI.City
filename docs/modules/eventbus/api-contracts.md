...
| EventDeliveryResult constructor/init | 创建单个订阅 delivery 结果。 | subscriptionId 必须已创建，dispatchPolicy 必须是已定义值，成功结果不得同时取消或携带 error message。 | EventDeliveryResult。 | constructor 或 init mutation 中 subscriptionId 为 default、成功且取消、成功且有错误消息抛 `ArgumentException`；未知 dispatchPolicy 抛 `ArgumentOutOfRangeException`。 | 无。 | 结果创建后不可变。 |

...
| EventDeliveryResult constructor | 创建单个订阅 delivery 结果。 | subscriptionId 必须已创建，dispatchPolicy 必须是已定义值，成功结果不得同时取消或携带 error message。 | EventDeliveryResult。 | subscriptionId 为 default、成功且取消、成功且有错误消息抛 `ArgumentException`；未知 dispatchPolicy 抛 `ArgumentOutOfRangeException`。 | 无。 | 结果创建后不可变。 |
| EventPostResult constructor | 创建 post 接受或拒绝结果。 | eventId 必须非空，contractId 必须已创建；Accepted 为 true 时不得有 rejection reason，Accepted 为 false 时必须有 rejection reason。 | EventPostResult。 | eventId 为空、contractId 为 default 或 accepted/rejection reason 状态不一致时抛 `ArgumentException`。 | 无。 | 结果创建后不可变。 |
...
| EventContext<TEvent> constructor | 创建 handler 执行上下文。 | eventData 非 null，contractId/subscriptionId 必须已创建，eventId 必须非空，correlationId 必须为稳定 id，不得为空白、包含首尾空白或控制字符，publishDepth 必须大于等于 0，dispatchPolicy 必须是已定义值。 | EventContext<TEvent>。 | eventData 为 null 抛 `ArgumentNullException`；contractId/subscriptionId 为 default、eventId 为空或 correlationId 无效抛 `ArgumentException`；publishDepth 小于 0 或未知 dispatchPolicy 抛 `ArgumentOutOfRangeException`。 | 仅保存 token，不主动取消。 | 创建后不可变。 |

...
| EventContext<TEvent> constructor | 创建 handler 执行上下文。 | eventData 非 null，contractId 必须已创建，correlationId 非空白。 | EventContext<TEvent>。 | eventData 为 null 抛 `ArgumentNullException`；contractId 为 default 或 correlationId 无效抛 `ArgumentException`。 | 仅保存 token，不主动取消。 | 创建后不可变。 |

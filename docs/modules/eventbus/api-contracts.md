...
| EventPostResult constructor/init | 创建 post 接受或拒绝结果。 | eventId 必须非空，contractId 必须已创建；Accepted 为 true 时不得有 rejection reason，Accepted 为 false 时必须有 rejection reason。 | EventPostResult。 | constructor 或 init mutation 中 eventId 为空、contractId 为 default 或 accepted/rejection reason 状态不一致时抛 `ArgumentException`。 | 无。 | 结果创建后不可变。 |

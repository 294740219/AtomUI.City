...
| EventPublishOptions | 描述 publish 上下文。 | `PublishDepth` 必须大于等于 0。 | EventPublishOptions。 | publish depth 小于 0 时，options init 和发布入口抛 `ArgumentOutOfRangeException`。 | 无。 | 调用内只读取，不回写 options。 |

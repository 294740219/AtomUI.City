...
| AUC-PLUGIN-006 | PluginLifecycle | PluginMsBuildContractTests | 断言 MSBuild property、output path、package content roots。 | 属性缺失、layout 不合法、manifest 未生成。 | Completed |
| AUC-PLUGIN-007 | PluginLifecycle | PluginResultTests | 断言 AUCPLG0000-0023 catalog 唯一、连续、不可变，且 diagnostic code/message 非空。 | 诊断码不能复用，context 必须有 pluginId/path。 | Completed |
| AUC-PLUGIN-008 | PluginLifecycle | PluginLoadingTests | 断言 Active -> Unloading -> Unloaded、lease revoke 和 UnloadPending diagnostics。 | active contribution、未释放 view/subscription/connection。 | Completed |
